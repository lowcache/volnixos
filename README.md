...
---
> This is my personal Nix Config for ny hardware. I would not recommend attempting to install on just any system. However, removal of Asus & other Hardware specific configurations will be made available under its own branch soon. 
---
...
* **Out-of-store Symlinks:** Dotfiles in user space are mapped via `config.lib.file.mkOutOfStoreSymlink` to the git repository under `/persist/home/nondeus/.nix-config/dots/`. This allows modifications in the repository (e.g., config changes) to take effect instantly without needing a full `home-manager switch`.
...
├─illogical-impulse/   # Rich dashboard config system (Roman workspaces, updates)
...
* System configurations are loaded via Matugen to extract colors directly from the active wallpaper for instantaneous interface ricing.
* Includes custom utilities like a screenshot tool (utilizing grim, slurp, and swappy) and custom overlays.

### 2.2 Advanced Work Safety Daemon
...
* **`flake.nix`**: System inputs, CachyOS kernel overlays, lanzarboote/sops imports, and system configurations mapping.
* **`nixos/`**:
...
* **`nxrbs`**: Runs a full NixOS system rebuild and applies changes on-the-fly from the local flake:
  ```bash
  sudo nixos-rebuild switch --flake /persist/home/nondeus/.nix-config/#nondeus
  ```
* **`nxrbb`**: Dry-builds the system configuration to test for compilations and evaluation failures:
  ```bash
  sudo nixos-rebuild build --flake /persist/home/nondeus/.nix-config/#nondeus
  ```
...
* **`priv-sync`**: Safely rsyncs live persistent directories (Documents, Pictures, repos, Gemini configs) from the ephemeral home directory space into the secure backing repository path:
  ...
  priv-sync
  ```
* **`setwall`**: Instantly changes wallpapers globally or on a specified monitor, dynamically running Matugen overlays to update the active Quickshell theme:
  ```bash
