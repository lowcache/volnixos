...
* **Brave/GTK File Chooser Failure — Root Cause: Hyprland Ambient CAP_SYS_NICE Blocks xdg-portal ptrace:**
  ...
  * **Status (2026-06-12 - OPTION A APPLIED):** Hyprland **0.55.3** (released 2026-06-08) now in pending nixpkgs rebuild (dry-run eval confirms in closure). Upstream issue xdg-desktop-portal#1691 and related discussions confirm CAP_SYS_NICE ptrace gate is known upstream. **Previous hypothesis (mistakes.md #10: dbus-broker pidfd):** Disproven — dbus-daemon 1.16.2 also passes pidfds; the 2026-06-09 "proof" succeeded only because the test was invoked **from a terminal** inheriting the same ambient cap as clients. Execution context was the variable, not bus implementation. `services.dbus.implementation = lib.mkForce "dbus"` workaround reverted (confirmed no-op; dbus-broker back as default per uwsm). **Post-rebuild verification pending (2026-06-12):** (1) `grep CapAmb /proc/$$/status` in fresh terminal → should be `0000000000000000`; (2) `gdbus call --session --dest org.freedesktop.portal.Desktop --object-path /org/freedesktop/portal/desktop --method org.freedesktop.portal.Settings.Read "org.freedesktop.appearance" "color-scheme"` → should return `(<<uint32 0>>,)` instead of AccessDenied; (3) Brave download / file-roller choosers; (4) `readlink ~/volnix` should exist (declaratively mapped in home/persist.nix).
...
* **Symlink Status (2026-06-12):** Declaratively mapped in `home/persist.nix`. Manual recreate on 2026-06-11 (post-reboot disappearance) with `ln -sn /persist/home/lowcache/.nix-config /home/lowcache/volnix`. Post-rebuild verification pending (activation timing vs. tether first-use).
