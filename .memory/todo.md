...
* [ ] **Resolve file-chooser/portal bug (CAP_SYS_NICE root cause):** Diagnosis complete (2026-06-12); real root cause is Hyprland ambient CAP_SYS_NICE blocking xdg-portal ptrace, **not** dbus-broker. **Status (2026-06-12):** Upstream research (tether task `portal-cap-upstream-research`) completed; xdg-desktop-portal#1691 confirms issue is known upstream. Three fix candidates: **(A) Update nixpkgs → Hyprland 0.55.3+** (released 2026-06-08, supports `HYPRLAND_NO_RT` flag to disable ambient caps; recommended first step); **(B) surgical—drop cap from wrapper** (`security.wrappers.Hyprland.capabilities = lib.mkForce "";`; loses SCHED_RR); **(C) patch xdg-portal locally** (modify to handle EACCES gracefully; preserves performance, adds maintenance). **Immediate workaround:** `setpriv --ambient-caps -all <app>`. **Action:** (1) User decides which fix path; (2) Apply chosen fix, rebuild, reboot; (3) Verify `gdbus call --session --dest org.freedesktop.portal.Desktop --object-path /org/freedesktop/portal/desktop --method org.freedesktop.portal.Settings.ReadAll '[]'` returns settings dict (not `AccessDenied`); (4) Verify Brave download dialog and file-roller archive operations work.

* [ ] **Verify Brave File Chooser Dialogue:** Open Brave browser, trigger a download or upload action, and verify that the GTK/Portal file picker window displays correctly and allows saving/loading files. **Status (2026-06-12):** Gated on CAP_SYS_NICE fix (see "Resolve file-chooser/portal bug" task). Workaround: `setpriv --ambient-caps -all brave` succeeds. Verification deferred until chosen fix candidate is applied and rebuilt.
