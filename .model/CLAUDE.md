...
* **Gotchas:** agy flags must precede `--print` (the wrapper handles it); agy can't register hidden dirs as workspace folders (this includes `~/.nix-config` itself) but doesn't resolve symlinks — delegate via `~/volnix`, the declarative non-hidden alias of the repo (`home/persist.nix`), which is the wrapper's default workdir.
